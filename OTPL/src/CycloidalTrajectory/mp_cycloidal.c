	...
	if (in->Tn[0]>TIME_ERR)
	{
		...
		if (t_N[0] >= 1)
		{
			...
			for (int i = cur_n; i< cur_n + t_N[0] + 1; i++)
			{
				...
				data[1][i] = DispTemp + in->h *(tau1 - 1.0f / (2.0 * M_PI)*sin(2 * w*tau1));
				data[2][i] = vecTemp + in->h / in->T*(1.0f - cos(2.0*w*tau1));
				data[3][i] = accTemp + 2 * M_PI*in->h / (in->T*in->T)*sin(2 * w*tau1);
				data[4][i] = jerkTemp + 4 * M_PI*M_PI*in->h / (in->T*in->T*in->T)*cos(2 * w*tau1);
			}
		...
		jerkTemp = jerkTemp + 4 * M_PI*M_PI*in->h / (in->T*in->T*in->T)*cos(2 * w*tau1);
